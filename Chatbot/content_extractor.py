...
def resolve_project_root(project_root: Optional[str] = None) -> str:
    """Resolve monorepo root (parent of Chatbot/) or explicit override."""
    if project_root:
...
def resolve_src_root(project_root: Optional[str] = None) -> Optional[str]:
    ...
    2. monorepo ../src
    3. bundled Chatbot/website_sources/src
    """
...
class ContentExtractor:
    ...
    def compute_content_fingerprint(self) -> str:
        ...
        for rel in sorted(CONTENT_VERSION_FILES):
            ...
            with open(path, "rb") as handle:
                data = handle.read()
            # Keep meaningful string content intact while ignoring checkout newline,
            # indentation, and blank-line-only changes.
            data = data.replace(b"\r\n", b"\n").replace(b"\r", b"\n")
            data = b"\n".join(
    ...
    def validate_documents(self, documents: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Validate against the current source arrays rather than a second,
        # hand-maintained backend list. This detects omissions and removals alike.
        required_entities = {
    ...
    def extract_team_members(self) -> List[Dict[str, Any]]:
        ...
        # The organizational hierarchy is derived only from the rendered About
        # page declarations.  Do not keep a second, hard-coded team roster here.
    ...
    def extract_service_catalogue(self) -> List[Dict[str, Any]]:
        ...
        `Services.tsx` is the public catalogue.  Routes determine whether a card
        is active, and each detail page supplies its translation namespace.  This
        avoids maintaining a duplicate backend service array or service count.
        """
        ...
        # Header navigation uses the same public translation namespace as the
        # active route and is available even in a minimal bundled source set.
    ...
    def _docs_localized_content(self) -> List[Dict[str, Any]]:
        ...
        They are intentionally separate from English entity documents so retrieval
        can filter/debug by language without duplicating an entity in one language.
        """
        documents: List[Dict[str, Any]] = []
